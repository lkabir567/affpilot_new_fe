import useFeatureList from "@/features/dashboard/overview/hooks/useFeatureList";
import { useGetFeatureListQuery } from "@/redux/api";
import { FileEdit, FileText, Layers, Sparkles, Users, Zap } from "lucide-react";
...
export function ToolsGrid() {
  const { data: tools } = useGetFeatureListQuery({});
  const { allFeatures } = useFeatureList();
  ...
  const features = [
    ...allFeatures,
    ...(tools?.results.length > 0 ? tools.results : []),
  ];

  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {features?.length > 0 &&
        features.map((tool) => <ToolCard key={tool.id} tool={tool} />)}
